...
export const StreamingTextDisplayI18n: React.FC<StreamingTextDisplayProps> = ({
  content,
  isStreaming,
  isComplete,
  showTypewriter = true,
  ...
  // Extract and format content from streaming JSON with translations
  ...
  // Content formatters with translations
  ...
  const formatInitialState = (parsed: any): { text: string; isJSON: boolean } => {
    ...
    if (parsed.childDescription) {
      text += `${labels.childDescription}\n${parsed.childDescription}\n`;
    }
    
    if (parsed.wealthTier) {
      const wealthText = getWealthText(parsed.wealthTier);
      text += `\n${labels.familyWealth} ${wealthText}\n`;
    }
  ...
  // Extract options from JSON content with translations
  ...
  // Extract initial state progressively with translations
  const extractInitialStateProgressive = (content: string): { text: string; isJSON: boolean } => {
    ...
    if (childName) {
      const genderText = childGender ? getGenderText(childGender as 'male' | 'female') : '';
      text += `${labels.child} ${childName}${childGender ? `（${genderText}）` : ''}\n\n`;
    }
  ...
  // Format text with styling (internationalized)
  const formatText = (text: string): React.ReactNode => {
    // const labels = getGameLabels(); // Available for future use
    ...
    return text.split('\n').map((line, index) => {
      ...
      if (line.includes('**')) {
        ...
        formattedLine = parts.map((part, i) => {
          if (i % 2 === 1) {
             ...
             const style = part === `${t('ui.question')}：` ? questionStyle : defaultStyle;
      ...
      // Style question text
      if (index > 0 && text.split('\n')[index - 1].includes(`**${t('ui.question')}：**`) && line.trim() && !line.includes('**')) {
        formattedLine = (
          <Typography
            component="div"
            sx={{
  ...
  if (!content && isStreaming) {
    const isOutcomePlaceholder = defaultPlaceholder.includes(t('ui.result', '结果'));
        ...
        <Fade in timeout={300}>
          <Box sx={{ minHeight: isOutcomePlaceholder ? 60 : 200 }}>
            ...
            {!isOutcomePlaceholder && (
              <Box sx={{ opacity: 0.3 }}>
                <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>{t('ui.question')}：</Typography>
                <Box sx={{ height: 60, mb: 3, backgroundColor: 'action.hover', borderRadius: 1 }} />
                <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>{t('ui.options')}：</Typography>
